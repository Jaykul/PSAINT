[CmdletBinding()]Param()
$Env:PSModulePath = "${Env:USERPROFILE}\Projects\Modules\TestingModules;${Env:USERPROFILE}\Projects\Modules;${Env:PSModulePath}"

Import-Module Reflection
Import-Module PowerTest -Force

## Let's Test ConvertFrom-PropertyString
## Our tests have to provide sample input, call the function, and compare it to the output we expected.

test "Lookahead Property Name Dupes" { 
   arrange {
      $TestString = @"
Name=Fred Jones
...
"@
   ...
   act {
      $Converted = ConvertFrom-PropertyString -InputObject $TestString -RecordSeparator "`n(?=Name=)"
   } 
   assert {
      Assert-PropertyEqual $ExpectedOutput $Converted
   }
}



test "colons and blank lines" { 
   arrange {
      $TestString =    @"
ID:12345
Type:Developer
...
"@
      ...
      $Properties = 'ID','Type','StartDate','Code','Name'
   }
   act {
      $Converted = ConvertFrom-PropertyString -InputObject $TestString -ValueSeparator ':' -RecordSeparator '\r\n\s*\r\n'
   } 
   assert {
      $different = Assert-PropertyEqual $ExpectedOutput $Converted
      Assert-That { !$different } -FailMessage "Output should match expected output:`n$($Different | Out-String)"
   }
}
...
## test "colons and blank lines" { 
##    arrange {
##       $FileSystem = New-RhinoMock Sample.FileSystem @{ 
##          Method = 'ReadAllText'
##          Return = "test text goes here"
##          WhenArguments = $Profile.CurrentUserAllHosts
##       }
##    }
##    act {
##       $reader = New-Object Sample.Reader $FileSystem
##       $reader.LoadFile( $Profile.CurrentUserAllHosts )
##    } 
##    assert {
##       19 -eq $reader.Content.Length
##       "test text goes here" -eq $reader.Content 
##    }
## }
# SIG # Begin signature block
# MIIRDAYJKoZIhvcNAQcCoIIQ/TCCEPkCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUFXCUjpjvxElBmPmkiVle3Rny
# MLWggg5CMIIHBjCCBO6gAwIBAgIBFTANBgkqhkiG9w0BAQUFADB9MQswCQYDVQQG
...
# AQQBgjcCAQsxDjAMBgorBgEEAYI3AgEWMCMGCSqGSIb3DQEJBDEWBBQZFIB+Iqj5
# ivfK8lWMNAzBqE4D2DANBgkqhkiG9w0BAQEFAASCAQALpJQ1g05ap9Ji4DRmKl93
# wXKZ4obFBcBthhMH/W8niH52L4XXCzD7OtsDojBH7l5qT+O1CQm7zyr2nLE/+CIu
# jijEYzCrAyKuyTRsr1n1YOXgx9Xy9+Lph8BGkGqT5mSYLuxdfr9KwvmbUuvdcDBd
# byHQ5ALauDMHxPgUqvDpGPbLVw5oziYrHDCfoYKCmnU9VGNT3QyZ5u3r+GXf12ne
# JhfmAKQ0XoxGrMsVznnxpiXX5mhgDKmTJfofKhQ1zRLI+dUk3sjmaWBI/RSspPNP
# j0JAwBj/3NX9CRFdfTawIVrpKV3Mh0GHzrXAxK25sXQS0T2XsqMBlsBz9XpE1H/t
